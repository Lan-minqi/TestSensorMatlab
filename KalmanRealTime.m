function [] = KalmanRealTime()
    ...
    if(interfaceObject.status=='open')
        disp('Connected');
    end
    while 1
       ...
       while (get(interfaceObject, 'BytesAvailable') > 0) 
          disp(i);
           ...
           for j = 1:1:3
               acc(j) = str2double(S(5+j));
           end
           for j = 1:1:3
               magn(j) = str2double(S(8+j));
           end
           ...
           if(i<=accF_Length+4)
                ...
                Magn(2,i)=magn(2);
                Magn(3,i)=magn(3);
                GyroRate(1,i)=(rot(1)-Offset(1,1));
                GyroRate(2,i)=(rot(2)-Offset(2,1));
                GyroRate(3,i)=(rot(3)-Offset(3,1));
    ...
    subplot(3,1,2);plot(t,Acc(2,:),'b',t,AccF(2,:),'r',t,Magn(2,:),'g',t,MagnF(2,:),'c');legend('AccY','AccFY','MagnY','MagnFY');grid;
    ...
    figure;
    subplot(3,1,1);plot(t,accWithout(1,1:acqSize));grid;legend('AccX-gx');xlabel('time (sec)');ylabel('Angle (deg)');
    subplot(3,1,2);plot(t,accWithout(2,1:acqSize));grid;legend('AccY-gy');xlabel('time (sec)');ylabel('Angle (deg)');
    subplot(3,1,3);plot(t,accWithout(3,1:acqSize));grid;legend('AccZ-gz');xlabel('time (sec)');ylabel('Angle (deg)');
end